...
import { Film, Home, LogOut, Play, Radio, Settings, Tv } from "lucide-react";
...
const navItems = [
  ...
  { href: "/music-videos", label: "Music Videos", icon: Radio },
];
    ...
    <Sidebar>
      <SidebarHeader className="border-b border-sidebar-border">
        <Link to="/" onClick={() => setOpenMobile(false)} className="flex items-center gap-2.5 px-2 py-1 font-semibold text-lg">
          <div className="size-8 rounded-lg flex items-center justify-center" style={{ background: "linear-gradient(135deg, #FF4D6A, #E0354D)" }}>
            <Tv className="size-4" style={{ color: "white" }} />
          </div>
          <span>Centillion Stream</span>
        </Link>
      </SidebarHeader>
      <SidebarContent>
        <SidebarGroup>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <SidebarMenuButton size="lg">
                  <Avatar className="size-8"><AvatarFallback style={{ backgroundColor: "#FF4D6A", color: "white" }} className="text-sm font-medium">{user?.name?.charAt(0).toUpperCase() || "U"}</AvatarFallback></Avatar>
